...
- aligns all of the k-mers in the query sequence to all of the k-mers in the homolog sequences. For each query k-mer, the best matching k-mer from each homolog is selected and recorded. The results are returned in a `PairkAln` object.
...
.. autofunction:: pairk.PairkAln

The dataframes can be accessed from the ``PairkAln`` object directly. For example:
