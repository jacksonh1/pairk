...
def score_pseudo_aln_columns(
    ...
    if mask is not None:
        seqlist = [seqlist[i] for i in range(len(seqlist)) if mask[i]]
    ...
    for position in subseq_df.index:
        background_scores.extend(
            score_pseudo_aln_columns(
                subseq_df.loc[position, :].to_list(), score_func=score_func, mask=mask
            )
        )
    return background_scores

def subseq_df_2_background_scores_rbm_penalty(
    subseq_df: pd.DataFrame,
    rbm_df: pd.DataFrame,
    score_func: Callable = cs.property_entropy,
    mask: list[bool] | None = None,
    ...
    background_scores = []
    k = len(subseq_df.iloc[0, 0])
    for position in subseq_df.index:
        rbm_orthoids = list(
            rbm_df.loc[position][rbm_df.loc[position] == True].index
        )
        if len(rbm_orthoids) == 0:
            seqs = [k*"-"]*len(subseq_df.columns)
        else:
            seqs = subseq_df[["reference_kmer"] + rbm_orthoids].loc[position, :].to_list()
            while len(seqs) < len(subseq_df.columns):
                seqs.append(k*"-")
        print(seqs)
        background_scores.extend(
            score_pseudo_aln_columns(
                seqs, score_func=score_func, mask=mask
            )
        )
    return background_scores
...
def matrix_json_2_pairwise_scores(
    ...
    '''
    for the hit, slice the subseq_df to only include entries where the reciprocal best match is True.
    Then for each kmer, for anywhere that the rbm is false, set the kmer to gaps.
    '''
    ...
    if reciprocal_best_match:
        ...
        rbm_df = rbm_df[["reference_kmer"] + rbm_orthoids]
    ...
    background_scores = subseq_df_2_background_scores_rbm_penalty(subseq_df, rbm_df, score_func=columnwise_score_func)
    # print(matrix_json)
    # for i in subseq_df.index:
    #     print(i, subseq_df.loc[i, "reference_kmer"])
